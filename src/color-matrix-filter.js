...
const ColorMatrixFilter = ({
  ...
  const concatedMatrix = parentMatrix
    ? concatTwoColorMatrices(mat, parentMatrix)
    : mat;
  const child = React.Children.only(children);

  getImageData = callback => {
    this.nativeFilter.getImageData(callback);
  };
...
const createFilter = key =>
  filterMap[key] ||
  (filter => ({ amount, ...restProps }) => (
    <ColorMatrixFilter matrix={filter(amount)} {...restProps} />
  ));
